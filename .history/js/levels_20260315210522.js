...
const LEVELS = [
...

  {
    ...
    validate(doc, code) {
      ...
      // Count opening and closing tags to ensure proper closure
      const trOpen = (code.match(/<tr>/g) || []).length;
      const trClose = (code.match(/<\/tr>/g) || []).length;
      const thOpen = (code.match(/<th>/g) || []).length;
      const thClose = (code.match(/<\/th>/g) || []).length;
      const tdOpen = (code.match(/<td>/g) || []).length;
      const tdClose = (code.match(/<\/td>/g) || []).length;
      
      return code.includes('<table>') && code.includes('</table>') &&
             trOpen === trClose && trOpen >= 3 &&
             thOpen === thClose && thOpen >= 2 &&
             tdOpen === tdClose && tdOpen >= 4 &&
             rows.length >= 3 && headers.length >= 2 && cells.length >= 4;
    }
    }
  },
